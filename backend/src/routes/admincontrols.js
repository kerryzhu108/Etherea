...
router.post("/themesTasks/:themeID", async(req, res) =>{
    try{
        ...
        var result = await pool.query("SELECT * FROM v_theme_task WHERE taskName=$1", [new_entry.taskName])
...
router.post("/themesAll", async(req, res) =>{
    try{
        ...
        var result = await pool.query("SELECT * FROM themes WHERE theme=$1", [new_entry.theme])
        if(result.rows.length > 0){
